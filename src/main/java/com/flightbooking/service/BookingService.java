...
public class BookingService {
 ...
 public ResponseEntity<ResponseStructure<Booking>> createbooking(Booking booking) {
        ...
        response.setMessage("Booking saved successfully ✈️✔️");
 ...
 public ResponseEntity<ResponseStructure<List<Booking>>> getallbooking() {
        ...
        if (!bookingDao.getallbooking().isEmpty()) {
            ...
            response.setMessage(" all booking records retrieved 📋✈️");
            response.setData(bookingDao.getallbooking());

            return new ResponseEntity<ResponseStructure<List<Booking>>>(response, HttpStatus.OK);
        } else {
            throw new Norecordavailableexception("no booking records are available ⚠️");
        }
    }
 ...
 public ResponseEntity<ResponseStructure<List<Booking>>> getbookingbydate(LocalDate bookingdate) {
        ...
        if (!bookingDao.getbookingbydate(bookingdate).isEmpty()) {
            response.setStatusCode(HttpStatus.OK.value());
            response.setMessage(" record with bookingdate" + bookingdate + " is retrieved 📅✔️");
 ...
 public ResponseEntity<ResponseStructure<List<Booking>>> getBookingsByFlightId(Integer flightId) {
        ...
        if (!(bookings.isEmpty())) {
            response.setMessage("Bookings with flightid" + flightId + "has been retrieved ✈️📋");
            response.setStatusCode(HttpStatus.OK.value());
 ...
 public ResponseEntity<ResponseStructure<List<Booking>>> getbookingbystatus(Bookingstatus bookingstatus) {
        ResponseStructure<List<Booking>> response = new ResponseStructure<List<Booking>>();
        if (!bookingDao.getbookingbystatus(bookingstatus).isEmpty()) {
            response.setStatusCode(HttpStatus.OK.value());
            response.setMessage(" record with bookingstatus" + bookingstatus + " is retrieved ✔️");
 ...
 public ResponseEntity<ResponseStructure<List<Passenger>>> getAllPassengersInBooking(Integer bookingId) {
        ...
        structure.setMessage("Passengers fetched successfully 👥✔️");
 ...
 public ResponseEntity<ResponseStructure<Payment>> getPaymentOfBooking(Integer id) {
        ...
        if (payment == null) {
            throw new Paymentnotfoundexception("No payment found for booking id " + id + " ⚠️");
        }

        ResponseStructure<Payment> structure = new ResponseStructure<Payment>();
        structure.setMessage("Payment details fetched successfully 💳✔️");
        structure.setStatusCode(HttpStatus.OK.value());
 ...
 public ResponseEntity<ResponseStructure<Booking>> updateBooking(Integer bookingId, Bookingstatus newStatus) {

        Booking updatedBooking = bookingDao.updateBooking(bookingId, newStatus);

        ResponseStructure<Booking> response = new ResponseStructure<>();
        response.setMessage("Booking status updated successfully 🔄✔️");
 ...
 public ResponseEntity<ResponseStructure<String>> deletebooking(Integer id) {
        ...
        if (opt.isPresent()) {
            response.setStatusCode(HttpStatus.OK.value());
            response.setMessage("booking record with id " + id + " is deleted 🗑️✔️");
            response.setData("success");
        } else {
            response.setStatusCode(HttpStatus.NOT_FOUND.value());
            response.setMessage("No booking record found with id " + id + " ❌");
 ...
 public ResponseEntity<ResponseStructure<Page<Booking>>> getBookingByPagingAndSorting(int pagenumber,int pagesize,String field) {
        ResponseStructure<Page<Booking>> response = new ResponseStructure<Page<Booking>>();
        response.setStatusCode(HttpStatus.OK.value());
        response.setMessage(" booking record sorted 📄📊");
        response.setData(bookingDao.getBookByPagingAndSorting(pagenumber, pagesize, field));
